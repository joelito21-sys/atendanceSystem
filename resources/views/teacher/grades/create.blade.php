    ...
    <x-slot name="header">
        <div class="flex flex-col gap-2">
            <h2 class="text-3xl font-black text-slate-900 tracking-tight">
                Add New <span class="text-gradient">Grade</span>
            </h2>
            <p class="text-slate-500 mt-1">Enter a grade for a student</p>
        </div>
    </x-slot>
        ...
        <div class="bg-white rounded-2xl border border-slate-100 shadow-xl p-8">
            <form action="{{ route('teacher.grades.store') }}" method="POST" class="space-y-6">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <!-- Student Selection -->
                    <div>
                        <label for="student_id" class="block text-xs font-black text-slate-400 uppercase tracking-widest mb-2">
                            Student <span class="text-rose-500">*</span>
                        </label>
                        <select id="student_id" name="student_id" required
                                class="w-full rounded-xl border-slate-200 focus:border-sky-500 focus:ring-sky-500 bg-slate-50/50 @error('student_id') border-rose-500 @enderror">
                ...
                <div>
                    ...
                    <textarea id="remarks" name="remarks" rows="2"
                              ...
                              placeholder="Add any comments or remarks...">{{ old('remarks') }}</textarea>
                    @error('remarks')
                        <p class="mt-1 text-xs text-rose-600 font-bold uppercase tracking-tight">{{ $message }}</p>
                    @enderror
                </div>
